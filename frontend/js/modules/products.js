...
<div class="modal" style="max-width:360px;">
  ...
  <div class="modal-body">
    <p class="text-sm">Current: <strong>${product.stock} ${product.unit}</strong></p>
    <div class="form-group mt-md">
      <label class="form-label">Adjustment (+ to add, - to deduct)</label>
      <input type="number" class="form-input" id="stock-adj" placeholder="e.g. +50">
    </div>
  ...
  const saveStockAdjustment = async (productId) => {
    ...
    try {
      await API.patch(`/products/${productId}/stock`, { adjustment });
  ...
  return {
    ...
    search: (val) => {
      ...
      this._t = setTimeout(() => loadProducts(1), 300);
    },
    filterCat: (id) => { currentCategory = id; loadProducts(1); },
    filterStatus: (s) => loadProducts(1)
  };
})();
