...
var LEDGER_PATH = import_path.default.join(process.env.HOME || process.env.USERPROFILE || ".", ".cbk_ledger_transactions.txt");
...
async function startServer() {
  ...
  app.post("/api/buy-load", async (req, res) => {
    ...
    await import_fs.default.promises.appendFile(LEDGER_PATH, logLine, "utf8");
  ...
  app.post("/api/dafox/cashin", async (req, res) => {
    ...
    try {
      ...
      const logLine = JSON.stringify({
        ...
        referenceId,
        routingStatus: "GATEWAY_CLEARED",
        // Updated since API was successfully hit (or simulated)
  ...
  app.get("/api/dafox/history", async (req, res) => {
    try {
      if (!import_fs.default.existsSync(LEDGER_PATH)) {
        return res.json({ success: true, data: [] });
      }
      const fileData = await import_fs.default.promises.readFile(LEDGER_PATH, "utf8");
