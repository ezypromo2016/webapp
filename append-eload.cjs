        ...
        {activeTab === "e-load" && (
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-1.5 relative">
                ...
                <div className="relative group">
                  ...
                  <input
                    type="text"
                    value={eloadPhone.slice(2)}
                    onChange={(e) => {
                      const val = e.target.value.replace(/[^0-9]/g, '');
                      if (val.length <= 9) setEloadPhone('09' + val);
                    }}
                    placeholder="171234567"
                    className="w-full bg-[#15161d] border border-white/5 rounded-2xl py-4 pl-16 pr-4 text-white text-lg font-black placeholder-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-mono tracking-widest"
                    required
                    disabled={!isOnline}
                  />
                </div>
            ...
            <button
              ...
              className={\`w-full py-5 \${isPadalaOnlyUser ? 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-600/20' : 'bg-white hover:bg-white/90 text-slate-900'} rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl transition-all font-sans disabled:opacity-50 flex items-center justify-center gap-3\`}
            >
              {isEloading ? (
                <>
...
// Insert the eLoadFormString right after the closing tag of the <motion.form> around line 897
code = code.replace(/<\/motion\.form>(\s*\{\/\* Transaction History Section \*\/)/, '</motion.form>\n' + eLoadFormString + '\n$1');

fs.writeFileSync(path, code);
console.log("Properly appended eLoad form.");
